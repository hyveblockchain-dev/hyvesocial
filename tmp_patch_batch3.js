// Patch script: Add batch 3 Discord features to server.js
// Run via: node tmp_patch_batch3.js | ssh root@157.250.207.109 'bash'
...
app.post('/api/channels/:channelId/messages/:messageId/save', authenticateToken, async (req, res) => {
  try {
    ...
    await db.query('INSERT INTO saved_messages (user_address, channel_id, message_id, note) VALUES ($1, $2, $3, $4)', [address, channelId, messageId, note || '']);
    res.json({ saved: true });
  } catch (err) {
...
app.patch('/api/users/activity', authenticateToken, async (req, res) => {
  try {
    ...
    io.emit('user_activity_update', { address, activityType, activityName, activityDetails });
...
// ═══════════════════════════════════════════════════════════
// ANNOUNCEMENT PUBLISH/CROSSPOST
// ═══════════════════════════════════════════════════════════
...
app.post('/api/channels/:channelId/messages/:messageId/publish', authenticateToken, async (req, res) => {
  try {
    const address = await resolveAddress(req.user.address);
    const { channelId, messageId } = req.params;
    // Verify channel is announcement type
...
// Output as a shell command to inject into server.js
console.log('Script generated - see the endpoints variable above');
